...
export default ({ $sentry, $axios, req, $config }) => {
    ...
    $axios.onRequest(config => {
        config.paramsSerializer = params => qs.stringify(Object.fromEntries(Object.entries(params)
            .filter(p => p[1] !== null)), { arrayFormat: 'repeat' });
        return config;
    });
};
